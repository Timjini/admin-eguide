  ...
  return (
    <>
     <Alert
      ...
      {showModal && (
        <div className="fixed top-0 left-0 right-0 bottom-0  opacity-50 z-40" />
      )}
      <div
        id="crud-modal"
        className={`${
          showModal ? 'fixed' : 'hidden'
        } top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 overflow-y-auto overflow-x-hidden z-50  rounded-lg shadow  w-full md:max-w-lg md:w-full`}
      >
        <div className="relative p-4 w-full ">
          <div className="relative ">
            <div className="flex items-center justify-between p-4 md:p-5 border-b rounded-t ">
              ...
              <button
                type="button"
                className="bg-transparent over: rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center "
                onClick={openModal}
              >
                <svg
                  className="w-3 h-3"
